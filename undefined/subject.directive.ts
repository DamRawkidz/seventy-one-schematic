...
import { Subject Service } from './Subject/Subject.service';

@Directive({
  selector: '[appSubject]'
})
export class SubjectDirective implements OnInit, OnDestroy  {
  ...
  ngOnInit(): void {
    fromEvent(this.element.nativeElement, 'input').pipe(
      ...
      switchMap(text => this.SubjectSV.queryString(`${text}`)),
